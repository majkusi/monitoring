...
while true; do
    ...
    if awk -v ram_pr="$ram_percentage" -v ram_th="$RAM_THRESHOLD" 'BEGIN{exit ram_pr<ram_th}'; then
        echo "$timestamp, RAM ALERT: ${ram_percentage}% >= ${RAM_THRESHOLD}%" >> /logs/alerts.log
    fi
    ...
    if [ "$disk_value" -gt "$DISK_THRESHOLD" ]; then
        echo "$timestamp,DISK ALERT: ${disk_value}% > ${DISK_THRESHOLD}%" >> /logs/alerts.log
    fi
    ...
    if awk -v cpu_pr="$cpu_percentage" -v cpu_th="$CPU_THRESHOLD" 'BEGIN{exit cpu_pr<cpu_th}'; then 
        echo "$timestamp,CPU ALERT: ${cpu_percentage}% > ${CPU_THRESHOLD}%" >> /logs/alerts.log
    fi
