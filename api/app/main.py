...
async def get_hourly_ram():
        ...
        return AvgHourlyRam(ram_pct=avg[0],ram_used=avg[1])
...
async def get_avg_hourly_cpu():
        ...
        return AvgHourlyCpu(cpu_pct=avg[0])

@app.get("/metrics/average/hourly/cpu", response_model=AvgHourlyDisk)
async def get_avg_hourly_disk():
      avg = db.execute(avg_per_hour(["DISK_PCT"]))
      return AvgHourlyDisk(disk_pct=avg[0])